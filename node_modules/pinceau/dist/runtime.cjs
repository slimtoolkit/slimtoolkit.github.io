...
function usePinceauRuntimeSheet($tokens, initialUtils = {}, colorSchemeMode, appId) {
  const sheet = vue.ref();
  const utils = vue.ref(initialUtils);
  const cache = {};
  function stringify$1(decl, loc) {
    return stringify.stringify(decl, (property, value, style, selectors) => stringify.resolveCssProperty(property, value, style, selectors, { $tokens, utils: utils.value, options: { colorSchemeMode } }, loc));
  }
...
function usePinceauRuntimeIds(instance, classes, _) {
  ...
  return vue.computed(() => ids);
}

function isToken(token) {
  return typeof token === "string" && stringify.keyRegex.test(token);
}
...
function usePinceauThemeSheet(initialTheme, tokensHelperConfig = {}) {
  const sheet = vue.ref();
  const theme = vue.ref(initialTheme || {});
  ...
  const $tokens = stringify.createTokensHelper(
    theme,
    tokensHelperConfig
  );
  let cache = {};
    ...
    Object.entries(cssRules || {}).forEach(
      ([_, rule]) => {
        ...
        rule.cssText.match(/--([\w-]+)\s*:\s*(.+?);/gm)?.forEach((match) => {
          ...
          data.set(theme.value, path, getSetValue(path, value, variable, currentTheme));
        });
      }
    );
  }
  function updateTheme(value) {
    ...
    const config = data.normalizeConfig(value || {}, mqKeys, true);
    data.walkTokens(config, (token, _, paths) => updateToken(paths, token.value));
  }
  function updateToken(path, value, mq = "initial") {
    ...
    const varName = stringify.pathToVarName(path);
    if (!cache?.[mq]) {
      createMqRule(mq);
    }
    const resolvedValue = stringify.resolveReferences(void 0, value, { $tokens });
    data.set(
      theme.value,
      path,
  ...
  function reactiveToken(path) {
    return vue.computed(
      {
        get() {
          return data.get(theme.value, `${path}.value`);
        },
        set(v) {
          updateToken(path, v);
        }
  ...
  function getSetValue(path, value, variable, mq = "initial") {
    ...
    const existingValue = data.get(theme.value, path);
    if (existingValue) {
  ...
  function createMqRule(mq) {
    ...
    let css;
    if (mqValue.match(stringify.responsiveMediaQueryRegex)) {
...
function usePinceauComputedStyles(ids, computedStyles, sheet, loc) {
  ...
  vue.onScopeDispose(() => rule && sheet.deleteRule(rule));
}
    ...
    for (const [varName, _value] of Object.entries(computedStyles)) {
      ...
      if (typeof value === "object") {
        for (const [mqId, mqPropValue] of Object.entries(value)) {
          const _value2 = vue.unref(mqPropValue);
          if (!_value2) {
            continue;
          }
          if (mqId === "initial") {
          ...
          declaration[mediaId][targetId][`--${scule.kebabCase(varName)}`] = _value2;
        }
      } else {
        const _value2 = vue.unref(value);
        if (_value2) {
          declaration[targetId][`--${scule.kebabCase(varName)}`] = _value2;
        }
      }
    }
  }
...
const usePinceauVariants = (ids, variants, props, sheet, classes, loc) => {
  ...
  const variantsState = vue.computed(() => variants && variants?.value ? resolveVariantsState(ids.value, props.value, variants.value) : {});
  vue.watch(
    variantsState,
    ({ cacheId, variantsProps }) => {
      ...
      if (sheet.cache[cacheId]) {
        ...
        variantClass = `pv-${nanoid.nanoid(6)}`;
      ...
      if (typeof propValue === "object") {
        for (const [mqId, mqPropValue] of Object.entries(propValue)) {
          ...
          if (mqId === "initial") {
            ...
            declaration[targetId] = defu.defu(declaration[targetId], variantValue);
          }
          const mediaId = `@${mqId}`;
          if (!declaration[mediaId]) {
            declaration[mediaId] = {};
          ...
          declaration[mediaId][targetId] = defu.defu(declaration[mediaId][targetId], variantValue);
        }
      } else {
...
function usePinceauCssProp(ids, props, sheet, loc) {
  ...
  const css = vue.computed(() => props.value?.css);
  vue.watch(
    css,
    (newCss) => {
...
const plugin = {
  install(app, {
    ...
    if (dev && (undefined || process.server)) {
      import('./chunks/debug.cjs').then(({ usePinceauRuntimeDebug }) => {
        usePinceauRuntimeDebug(tokensHelperConfig);
      });
    }
    const multiAppId = multiApp ? nanoid.nanoid(6) : void 0;
    ...
    function usePinceauRuntime(props, variants, computedStyles) {
      ...
      return {
        $pinceau: vue.computed(() => `${classes.value.v} ${classes.value.c}`)
      };
    }
...
function usePinceauRuntime(props, variants, computedStyles) {
  return vue.inject("pinceauRuntime")(props, variants, computedStyles);
}
function usePinceauTheme() {
  return vue.inject("pinceauTheme");
}
const cssProp = {
  type: Object,
  required: false,
...
exports.scale = scale;
exports.usePinceauRuntime = usePinceauRuntime;
exports.usePinceauTheme = usePinceauTheme;
exports.utils = utils;
